...
def main():
    ...
    data = data['W'] * ref_data.rho.squeeze()  # convert to mass flux
    ...
    print('Saving output ...')
    model.components().to_netcdf(f"{configs.get_project_root_dir()}/data-output/gsam.eofs_era5_levels.{REGION_NAME}.massflux.50pix.nc")
    model.scores(normalized=False).unstack().to_netcdf(f"{configs.get_project_root_dir()}/data-output/gsam.pcs_era5_levels.{REGION_NAME}.massflux.50pix.nc")
    print('Done!')
