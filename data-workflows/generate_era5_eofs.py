...
## CURRENTLY CONFIGURED FOR MASS FLUX ##
...
def main():
    ...
    model.components().to_netcdf(f"{configs.get_project_root_dir()}/generated_data/era5.eofs.{REGION_NAME}.massflux.2deg.nc")
    model.scores(normalized=False).unstack().to_netcdf(f"{configs.get_project_root_dir()}/generated_data/era5.pcs.{REGION_NAME}.massflux.2deg.nc")
