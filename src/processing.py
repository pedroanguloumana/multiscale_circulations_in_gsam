...
class MoistureSpaceGrids:
    ...
    def _get_moisturespace_filename(self, region, lati, loni, time):
        fn_pattern = f'{gSAMConfigs().moisture_space_var_dir}satfrac_sorted.lon_{loni}.lat_{lati}*{time}_*.nc'
        file = glob(fn_pattern)
        assert(len(file)==1)
        return(file[0])
        
    def composite_moisture_grids(self, region, variable, rmin=0.5):
        pcs_file = f'{ProjectConfigs().project_root_dir}/data/gsam.eofs.{region}.massflux.50pix.nc'
        ...
        for p in [1,2,3,4,5,6,7,8]:
            ...
            fnames = [self._get_moisturespace_filename(region, lati, loni, time_string) for lati, loni, ti in zip(lat_idx, lon_idx, time_idx)]
            print('Concating grid from Phase {p}...')
            phase_data = xr.concat([xr.open_dataarray(_) for _ in fnames], dim='observations')
            breakpoint()
            # mass_flux = (phase_data - phase_data.mean('column'))
